...
- **A related group of scripts gets its own subfolder** — `python .claude/scripts/<dir>/<name>.py`, e.g. `.claude/scripts/tft/`. Anything with its own data, archive, or helper module belongs in one, so that generic names (`data/`, `archive/`) cannot collide with another group's. The rule's real point is that nothing custom lives at the **project root** — that still holds.
